function [zbest,BestFit,net] = EGPSO(pop,V,maxgen,hiddennum,input_train1, output_train1,inputnum,outputnum,popmin,popmax,Vmin,Vmax)
...
%%  迭代寻优
for i = 1 : maxgen
    pop=EGolden(pop,sizepop,numsum,zbest,hiddennum,net,input_train1, output_train1);
    for j = 1 : sizepop
    ...
    for j = 1 : sizepop
        ...
        if fitness(j) < fitnessgbest(j)
            ...
            newpop=levy(pop(j,:),popmin,popmax);
            if fun(newpop,hiddennum,net,input_train1,output_train1)<fitness(j)
                pop(j,:)=newpop;
            end
        end
